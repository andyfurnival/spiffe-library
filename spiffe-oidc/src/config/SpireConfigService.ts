...
import { AdminConfig } from 'ts-spiffe/src/config';
import { Injectable } from '@nestjs/common';
import { WorkloadConfig } from 'ts-spiffe/src';
...
export class SpireConfigService {
  ...
  getIdentityConfig(): WorkloadConfig {
    return {
      spireEndpoint: this.configService.get<string>(
        'DELEGATED_ID_SPIRE_ENDPOINT',
      ),
      reconnectInterval: this.configService.get<number>(
        'DELEGATED_ID_RECONNECT_INTERVAL',
      ),
    };
  }
}
